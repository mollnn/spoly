...
public:
	...
	void getRayDifferential(const Ray & ray, RayDifferential & rayDiff) const {
		rayDiff = RayDifferential(ray);
		const Transform &trafo = m_worldTransform->eval(ray.time);
		const Transform &trafoInv = trafo.inverse();
		Point nearP(Point(trafoInv(ray.d)));
		Point screenSample = m_cameraToSample(nearP);
		Point nearPx = m_sampleToCamera(screenSample + Point(m_invResolution.x, 0, 0));
		Point nearPy = m_sampleToCamera(screenSample + Point(0, m_invResolution.y, 0));


		rayDiff.rxOrigin = rayDiff.ryOrigin = ray.o;
		rayDiff.rxDirection = trafo(normalize(Vector(nearPx)));
		rayDiff.ryDirection = trafo(normalize(Vector(nearPy)));
