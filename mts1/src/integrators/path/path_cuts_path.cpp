...
public:
    PathCutPathTracer(const Properties &props)
        ...
        m_useDepthCheck = props.getBoolean("useDepthCheck", false);
        m_pathType = props.getInteger("pathType", 0);
        pathType_glb = m_pathType;
        ...
        Log(EInfo, "m_useDepthCheck %d", m_useDepthCheck);
        Log(EInfo, "m_pathType %d", m_pathType);
    ...
    Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        ...
        while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
            ...
            /* Depth Check */
            int depth_calibration = (m_useDepthCheck)?1:0;
            if ((rRec.depth + depth_calibration >= m_maxDepth && m_maxDepth > 0)
            ...
            if (rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance &&
                (bsdf->getType() & BSDF::ESmooth) && !(bsdf->getType() & BSDF::EDelta)) {
            ...
            if (bsdf->getType() & BSDF::ESmooth) {
                ...
                auto ans = bgr->renderOneImage("", m_pathcutBounce, 1, m_pathcutThres, false);
                Spectrum value(0.0);
                for (auto [d, v]: ans) {
            ...
            if(!hitEmitter || !(bsdf->getType() & BSDF::EDelta))
            {
    ...
    bool m_useDepthCheck = false;
    float m_pathcutThres = 0.000001;
    int m_pathType = 0; // 0 is reflection && 1 is refraction 
